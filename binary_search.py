...
def recursive_binary_search(seq, number, ixl, ixr):
    nb_val = len(list)
    middle = (ixl + ixr) // 2
    if number == seq[middle]:
        return middle
        ...
        return recursive_binary_search(seq, number, ixl, middle - 1)
    elif number > seq[middle]:
        return recursive_binary_search(seq, number, middle + 1, ixr)
